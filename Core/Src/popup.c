...
bool popup_mgr_register(popup_base_t *p) {
    ...
    for (uint8_t i = 0; i < g_popup_count; i++)
        if (g_popup_list[i] == p) return true;
...
static void popup_num_render(const popup_num_t *p, u8g2_t *u8g2) {
    ...
    if (range > 0) {
        ...
        int16_t fill_w = (int16_t)((int32_t)(*p->cfg.value - p->cfg.min) * max_fill / range);
...
static bool toast_active(void *p)  { return ((popup_toast_t *)p)->state != POPUP_IDLE; }
static void toast_upd(void *p, int8_t key) { (void)key; popup_toast_update((popup_toast_t *)p); }
static void toast_rndr(void *p, u8g2_t *u8g2) { popup_toast_render((popup_toast_t *)p, u8g2); }

void popup_toast_init(popup_toast_t *p, popup_base_t *b) {
    p->state = POPUP_IDLE;
    anim_init(&p->slide);
    b->instance = p;
    b->active   = toast_active;
    b->update   = toast_upd;
    b->render   = toast_rndr;
    popup_mgr_register(b);
}
